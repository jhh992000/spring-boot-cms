...
export default {
  mounted() {
    ...
    function j(t, e, o, i) {
      ...
      let p; const l = window.getComputedStyle(t); const n = parseFloat(l.getPropertyValue("height")); const a = parseFloat(l.getPropertyValue("padding-top")); const s = parseFloat(l.getPropertyValue("padding-bottom")); const r = parseFloat(l.getPropertyValue("margin-top")); const d = parseFloat(l.getPropertyValue("margin-bottom")); const g = n / e; const y = a / e; const m = s / e; const u = r / e; const h = d / e;
      window.requestAnimationFrame(function l(x) {
        void 0 === p && (p = x);
        const f = x - p;
        i ? (t.style.height = g * f + "px", t.style.paddingTop = y * f + "px", t.style.paddingBottom = m * f + "px", t.style.marginTop = u * f + "px", t.style.marginBottom = h * f + "px") : (t.style.height = n - g * f + "px", t.style.paddingTop = a - y * f + "px", t.style.paddingBottom = s - m * f + "px", t.style.marginTop = r - u * f + "px", t.style.marginBottom = d - h * f + "px"), f >= e
            ? (t.style.height = "", t.style.paddingTop = "", t.style.paddingBottom = "", t.style.marginTop = "", t.style.marginBottom = "", t.style.overflow = "", i || (t.style.display = "none"), typeof o === "function" && o()) : window.requestAnimationFrame(l)
      })
    }

<template>
  <v-form v-model="valid">
    ...
    <div class="layout ma-0 row">
      <a class="mr-4">
        <v-icon> fa-4x fa-credit-card</v-icon>
      </a>

      <a class="mr-4">
        <v-icon> fa-4x fa-apple</v-icon>
      </a>

      <a class="mr-4">
        <v-icon> fa-4x fa-paypal</v-icon>
      </a>
    </div>
    <v-subheader class="pa-0 mt-3">Payment Detail</v-subheader>
    <div class="d-flex my-2">
      <v-select :items="cardTypes" v-model="form.cardTypeId" label="Card Type" auto required item-text="name" item-value="id"></v-select>
    </div>
    <div class="d-flex my-2">
      <v-text-field
        label="Card Number"

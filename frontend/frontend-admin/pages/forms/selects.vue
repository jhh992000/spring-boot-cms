            ...
            <div slot="widget-content">
              <v-container fluid>
                <v-layout row wrap>
                  ...
                  <v-flex xs6>
                    <v-select :items="countries" v-model="e1" label="Select" item-text="country" item-value="abbr" single-line></v-select>
                  </v-flex>
                  <v-flex xs6>
                  ...
                  <v-flex xs6>
                    <v-select label="Select" prepend-icon="map" :items="countries" v-model="e5" item-text="country" item-value="abbr"></v-select>
                  </v-flex>
                  <v-flex xs6>
                    <v-subheader>Append Icon</v-subheader>
                  </v-flex>
                  <v-flex xs6>
                    <v-select label="Select" :items="countries" v-model="e6" append-icon="map" item-text="country" item-value="abbr"></v-select>
                  </v-flex>
                  <v-flex xs6>
                    <v-subheader>Auto complete</v-subheader>
                  </v-flex>
                  <v-flex xs6>
                    <v-select label="Select" :items="countries" v-model="e7" autocomplete item-text="country" item-value="abbr"></v-select>
                  </v-flex>
                </v-layout>
              </v-container>
            </div>
    ...
    querySelections(v) {
      ...
      setTimeout(() => {
        this.items = this.countries.filter(e => {
          return (e.country || '').toLowerCase().includes((v || '').toLowerCase());
        });
        this.loading = false;
      }, 500);
    },
